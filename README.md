...
- **自动老化与固定保留**：项目默认 24 小时后清理；需要长期放着的内容可固定保留，不参与自动清理。
- **批量工作流**：进入批量选择后，可一次固定、恢复自动清理或移除多项内容，并支持撤销。
- **高度个性化**：可调停靠侧、失焦隐匿节奏、主题、强调色、界面密度、浮层通透度及动画强度。
- **命令面板**：用 `Ctrl + K` 快速触达添加、文本暂存、筛选、批量选择、清理与偏好。
...
3. 需要继续使用时，直接拖拽项目左侧抓手到另一个应用；也可展开项目后打开、定位、固定保留、复制分享链接或移除。

常用快捷键：`Ctrl + Shift + Space` 呼出或收起暂存架，`Ctrl + O` 选择文件，`Ctrl + N` 暂存文本，`Ctrl + F` 聚焦筛选，`Ctrl + K` 打开命令面板，`Esc` 收起当前浮层。
